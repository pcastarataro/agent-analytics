# Delta for monorepo-scaffold

> New capability: `openspec/specs/monorepo-scaffold/` does not exist yet. On archive, ALL requirements below are appended as the initial main spec.
...
The change SHALL initialize a git repository with a `.gitignore` excluding `node_modules/`, build output (`dist/`), coverage output, and local environment files. Repository history MUST begin with exactly ONE initial commit using a conventional-commits message containing the whole scaffold.

#### Scenario: History starts with one clean commit

- GIVEN bootstrap completed
- WHEN `git log --oneline` runs
- THEN exactly one commit exists with a conventional-commit message and the full scaffold tracked
