...
The system SHALL expose `GET /v1/stats/agents` returning per-agent aggregated metrics. The response MUST include an array of objects, each with: `agentName`, `version`, `executionCount`, `successRate` (0–100 percentage), `avgDurationMs`, `avgCost` (number), and `totalCost`. The endpoint MAY accept date-range filters (`from`, `to`) matching `GET /v1/events`. Aggregation MUST group by `(agentName, version)`.
