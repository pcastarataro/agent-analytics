...
import type { BranchDetail } from '../api/types';
...
export function BranchDetailPage() {
  ...
  const { data, loading, error, refetch } = useApi<BranchDetail>(
    `/v1/stats/branches/${encodeURIComponent(name ?? '')}`,
  );
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <Link to="/branches" className="text-sm text-blue-600 hover:underline">
          ← Branches
        </Link>
        <h2 className="text-xl font-bold text-gray-900">{data.branch}</h2>
      </div>
